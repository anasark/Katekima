...
const updateUrlParams = () => {
  const params: Record<string, string> = {
    search: search.value,
    limit: selectedLimit.value.toString(),
    offset: offsetParam.value.toString(),
  };
...
const goToEdit = (url: string) => router.push(`list-table/edit/${getIdFromUrl(url)}`);
const deleteBerry = (url: string) => alert(`Hapus ${getIdFromUrl(url)}?`);
</script>

<template>
  <div class="relative overflow-x-auto sm:rounded-lg">
    <div class="flex justify-end">
      <router-link to="/list-table/add" class="inline-flex items-center bg-rose-500 hover:bg-rose-600 border border-rose-300 cursor-pointer text-white px-3 py-1.5 mb-4 text-sm rounded-lg">Tambah</router-link>
    </div>
      ...
      <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody v-if="filteredBerries.length">
          <tr v-for="(berry, index) in filteredBerries" :key="index" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
            ...
            <td class="px-6 py-4">
              <a href="#" class="font-medium text-blue-600 dark:text-blue-500 hover:underline mx-2" @click="goToEdit(berry.url)">Edit</a>
              <a href="#" class="font-medium text-red-600 dark:text-red-500 hover:underline mx-2" @click="deleteBerry(berry.url)">Delete</a>
            </td>
          </tr>
        </tbody>
        <tbody v-else>
